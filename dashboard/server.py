...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_url.path == "/api/builder/download_bundle":
            q = urllib.parse.parse_qs(parsed_url.query)
            def q_get(k, default):
                vals = q.get(k, [])
                return vals[0] if vals else default

            model_name = q_get("name", "Custom_MoE").replace(" ", "_").replace("-", "_")
            specs = ModelArchitectureSpecs(
                name=model_name,
                d_model=int(q_get("d_model", 2048)),
                n_layers=int(q_get("n_layers", 24)),
                n_heads=int(q_get("n_heads", 16)),
                num_experts=int(q_get("num_experts", 12)),
                top_k=int(q_get("top_k", 2)),
                ffn_multiplier=float(q_get("ffn_multiplier", 2.0)),
                vocab_size=int(q_get("vocab_size", 1048576)),
                rank_embedding=int(q_get("rank_embedding", 64)),
                max_seq_len=int(q_get("max_seq_len", 7168)),
                expert_domains={},
                guardrails={
                    "rlvr_strictness": float(q_get("rlvr_strictness", 0.85)),
                    "hallucination_guard": float(q_get("hallucination_guard", 0.90)),
                    "epistemic_curiosity": float(q_get("epistemic_curiosity", 0.75)),
                },
            )
